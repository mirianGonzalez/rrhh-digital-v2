...
def convertir_a_pdf_seguro(archivo_entrada):
    """Convierte cualquier archivo a PDF de forma segura"""
    try:
        ...
        if extension == '.pdf':
            ...
            convert(archivo_entrada, nombre_salida)
            return nombre_salida
...
def init_db():
    ...
    cur.execute("""
        CREATE TABLE IF NOT EXISTS ddjj_conyuge(
            id INTEGER PRIMARY KEY AUTOINCREMENT,
            ddjj_id INTEGER NOT NULL UNIQUE,
            apellido_nombre TEXT,
            dni TEXT,
            fecha_enlace TEXT,
            lugar_enlace TEXT,
            ...
            razon_social TEXT,
            percibe_asignaciones INTEGER DEFAULT 0,
            observaciones TEXT,
            FOREIGN KEY(ddjj_id) REFERENCES declaraciones_juradas(id)
        )
    """)
    ...
    cur.execute("CREATE INDEX IF NOT EXISTS idx_legajos_dni ON legajos(dni)")
    cur.execute("CREATE INDEX IF NOT EXISTS idx_legajos_cuil ON legajos(cuil)")
    cur.execute("CREATE INDEX IF NOT EXISTS idx_documentos_legajo ON documentos(legajo)")
    cur.execute("CREATE INDEX IF NOT EXISTS idx_documentos_tipo ON documentos(tipo)")
    cur.execute("CREATE INDEX IF NOT EXISTS idx_documentos_hash ON documentos(hash_archivo)")
    ...
    cur.execute("CREATE INDEX IF NOT EXISTS idx_auditoria_fecha ON auditoria(fecha)")
...
def listar_legajos():
    ...
    return render_template("lista_legajos.html", legajos=legajos)
...
def listar_ddjj():
    ...
    cur.execute("""
        ...
        FROM declaraciones_juradas d
        JOIN legajos l ON d.legajo_id = l.legajo_id
        ORDER BY d.anio DESC, d.fecha_generacion DESC
    """)
    ddjj_list = cur.fetchall()
    return render_template("ddjj_lista.html", ddjj=ddjj_list)
...
def validar_qr(codigo):
    ...
    if not ddjj:
        return render_template("validacion_no_encontrada.html", codigo=codigo)
...
def ver_ddjj(ddjj_id):
    ...
    return send_file(ddjj['archivo_pdf'], as_attachment=False)
...
def generar_pdf_ddjj_auto(ddjj_id):
    """
    Genera PDF de una DDJJ (incluso si ya está finalizada)
    """
    conn = get_db()
    cur = conn.cursor()
    ...
    try:
        ...
        return send_file(pdf_final, as_attachment=False, 
                        download_name=f"DDJJ_{legajo['legajo_id']}_{ddjj['anio']}.pdf")

    except Exception as e:
        import traceback
        error_trace = traceback.format_exc()
        logger.error(f"Error en generación automática: {str(e)}\n{error_trace}")
        return f"<h1>❌ Error durante la generación</h1><pre>{str(e)}\n{error_trace}</pre>"
...
# ==========================
# RUTA PARA EDITAR LEGAJO (si no existe)
# ==========================
...
def editar_legajo(legajo_id):
    ...
    if request.method == "POST":
        ...
        """, (
            ...
            request.form.get('domicilio_calle'),
            request.form.get('domicilio_numero'),
        ...
        <div class="container">
            ...
            <div class="success">
                ...
                <ul>
                    ...
                    <li><strong>Domicilio:</strong> BOLIVAR 1588</li>
...
## ==========================
# crear la función listar_ddjj
# ==========================
@app.route('/listar_ddjj')
@login_requerido
def listar_ddjj():
    """Listado de todas las DDJJ usando la tabla correcta"""
    try:
        conn = get_db()
        cur = conn.cursor()
        
        # Usar la tabla correcta: declaraciones_juradas
        cur.execute('''
            SELECT 
                d.id,
                d.legajo_id as legajo,
                d.anio,
                d.estado,
                d.fecha_generacion as fecha_presentacion,
                d.archivo_pdf as archivo_path,
                l.apellido,
                l.nombre
            FROM declaraciones_juradas d
            LEFT JOIN legajos l ON d.legajo_id = l.legajo_id
            ORDER BY d.anio DESC, d.fecha_generacion DESC
        ''')
        ...
        return render_template('ddjj_lista.html', ddjj=lista_ddjj)
        
    except Exception as e:
        logger.error(f"Error al listar DDJJ: {e}")
        import traceback
        traceback.print_exc()
        flash(f'Error al cargar las DDJJ: {str(e)}', 'error')
        return redirect(url_for('panel'))
...
@login_requerido
def ver_ddjj_detalle(ddjj_id):
    """Ver detalle de una DDJJ específica"""
    try:
        conn = get_db()
        cur = conn.cursor()
        ...
        return render_template('ddjj_detalle.html', ddjj=datos_ddjj)
        
    except Exception as e:
        logger.error(f"Error al ver DDJJ: {e}")
        flash('Error al cargar la DDJJ', 'error')
        return redirect(url_for('listar_ddjj'))

@app.route('/ddjj/descargar/<int:id>')
@login_requerido
def descargar_archivo_ddjj(id):
    """Descargar archivo de DDJJ"""
    try:
        conn = get_db()
        cur = conn.cursor()
        cur.execute('SELECT archivo_pdf FROM declaraciones_juradas WHERE id = ?', (id,))
        result = cur.fetchone()
        
        if result and result['archivo_pdf']:
            archivo_path = result['archivo_pdf']
            if os.path.exists(archivo_path):
                return send_file(archivo_path, as_attachment=True, download_name=f"DDJJ_{id}.pdf")
            else:
                flash('Archivo no encontrado en el servidor', 'error')
        else:
            flash('DDJJ sin archivo adjunto', 'error')
            
        return redirect(url_for('listar_ddjj'))
        
    except Exception as e:
        logger.error(f"Error al descargar DDJJ: {e}")
...
if __name__ == "__main__":
    print("\n" + "="*60)
    print("🚀 SISTEMA DE GESTIÓN DE LEGAJOS - CON FIRMA DIGITAL PKI")
    print("="*60 + "\n")
    ...
    print("\n" + "="*60)
    print("✅ SISTEMA LISTO - REALIZADO POR M.Y.G.")
    print(f"🌐 http://127.0.0.1:5000")
    print(f"👤 Usuario admin: admin / Admin2026!")
    print(f"👤 Usuario director: director / Director2026!")
    print("="*60)
    print("\n📌 FLUJO DE FIRMA DIGITAL:")
    print("   1. AC interna emite certificado .p12 para cada agente")
    print("   2. Agente completa DDJJ y firma con PIN")
    print("   3. Director recibe notificación y co-firma")
    print("   4. DDJJ con doble firma tiene validez legal")
    print("="*60 + "\n")
